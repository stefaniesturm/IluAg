...
%% NEW PLOTS JANUARY 2022 by Stefanie

% Calculate the difference between the conditions of interest
load('GA_NOc_ERP.mat')
load('GA_YESc_ERP.mat')
cfg    = [];
avg_yes = ft_timelockanalysis(cfg, GA_YESc_ERP);
avg_no  = ft_timelockanalysis(cfg, GA_NOc_ERP);

% Then take the difference of the averages using ft_math
cfg           = [];
cfg.operation = 'subtract';
cfg.parameter = 'avg';
raweffectFICvsFC = ft_math(cfg, avg_yes, avg_no);

% Make a vector of all p-values associated with the clusters from ft_timelockstatistics.
pos_cluster_pvals = [yes_no.posclusters(:).prob];
...
pos       = ismember(yes_no.posclusterslabelmat, pos_clust);

% and now for the negative clusters...
neg_cluster_pvals = [yes_no.negclusters(:).prob];
neg_clust         = find(neg_cluster_pvals < 0.025);
neg               = ismember(yes_no.negclusterslabelmat, neg_clust);

timestep      = 0.05; % timestep between time windows for each subplot (in seconds)
sampling_rate = NO{1,1}.fsample; % Data has a temporal resolution of 300 Hz
sample_count  = length(yes_no.time);
...
% First ensure the channels to have the same order in the average and in the statistical output.
% This might not be the case, because ft_math might shuffle the order
[i1,i2] = match_str(raweffectYESvsNO.label, yes_no.label);
...
for k = 1:9
   ...
   pos_int = zeros(numel(raweffectYESvsNO.label),1);
   neg_int = zeros(numel(raweffectYESvsNO.label),1);
   pos_int(i1) = all(pos(i2, m(k):m(k+1)), 2);
   neg_int(i1) = all(neg(i2, m(k):m(k+1)), 2);
   ...
   cfg.layout      = layout2;
   cfg.interactive = 'no';
   ft_topoplotER(cfg, raweffectYESvsNO);
end
%%
%% Create plots for encoding
cd(indir); load GA_YESc_ERP; load GA_NOc_ERP;

cfg = [];
cfg.operation = 'subtract';
cfg.parameter = 'avg';
GA_YESvsNO = ft_math(cfg, GA_YESc_ERP, GA_NOc_ERP)

figure;
% define params for plotting 
timestep = 0.03 %in secs
sampling_rate = 500; %
sample_count = length(yes_no.time);
% j and M must have the same temporal length
j = [yes_no.time(1):timestep:yes_no.time(end)]; %temporal endpoints (in secs) of the ERP avg computed in each subplot
m = [1:timestep*sampling_rate:sample_count]; %temporal endpoints in EEG samples
% get relevant (significant) values for positive
pos_cluster_pvals = [yes_no.posclusters(:).prob];
pos_signif_clust = find(pos_cluster_pvals < yes_no.cfg.alpha);
pos = ismember(yes_no.posclusterslabelmat, pos_signif_clust);

% get relevant (significant) values for negative
neg_cluster_pvals = [yes_no.negclusters(:).prob];
neg_signif_clust = find(neg_cluster_pvals < yes_no.cfg.alpha);
neg = ismember(yes_no.negclusterslabelmat, neg_signif_clust);

% First ensure the channels to have the same order in the avg and in the
% stat output. This might not be the case, because ft_math might shuffle
% the order
[i1,i2] = match_str(GA_YESvsNO.label, yes_no.label);
...
for k = 1:length(m)-1
   ...
   % cfg.zlim = [-5e-14 5e-14];
    cfg.colorbar = 'EastOutside';
    pos_int = zeros(numel(GA_YESvsNO.label),1);
    pos_int(i1) = all(pos(i2, m(k):m(k+1)),2);
    neg_int = zeros(numel(GA_YESvsNO.label),1);
    neg_int(i1) = all(neg(i2, m(k):m(k+1)),2);
    cfg.highlight = 'on';
    cfg.highlightchannel = find(pos_int | neg_int);
