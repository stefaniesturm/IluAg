function IoA(Subj,Block,Repetition,counterbalancing)
%
% 2+    Illusion of Agency experiment, Bachelor thesis of Marta Blasco and
...
% in every trial and a sound contingent to t1234567890'¡12345snb,asdc
% 696
% 704  t he4 f2qesdarfcxyj vinm `.
% ´/hat button press is generated.
%
...
%
% TRIALS PER BLOCK
% There will be 15 blocks of 64 acquisition trials and 16 test trials.
...
Phase = 'Exp' %acquisition training (ATrain), main training (MTrain), experimental block (Exp) or motor-only block (MO)

% Timing vars
RespWinDur = 0.8; % From fixation, we check for presses for 800 ms. Afterwards fixation dissapears
RTlimits = [0.42 0.58]; % RTs considered "correct" between 420 and 580 ms. If pressed earlier cross turns orange, if later (or no resp) cross turn red, if on time, cross turns green
...
res = [50 50 600 600]; % screen resolution for display window for debugging (comment for fullscreen)
...
elseif counterbalancing == 2 % then control button produces low sound
...
SH = audioread ([Sounds_path '\SH_IA.wav']);
SL = audioread ([Sounds_path '\SL_IA.wav']);
...
nTTrials = 16; % Test trials
nATrials = 64; % Adquisition trials
...
nSequences= 8; % 5 sequences of each type (=75 trials total)
...
[EXPWIN] = Screen('OpenWindow',0,backcolor, res); % Open fullscreen
...
% Info we want to collect in Log to give feedback: 1) which button was pressed, 2) what was the RT
...
counter=0; %to know the test trial position
    ...
    while GetSecs < (FixTimeON + RespWinDur)
        ...
        if isempty(Button)
            ...
            if sum(keyCode(SoundButtonTypes_LogicalArray))
                ...
                Sound = find(Button == SoundButtonTypes); % 1 (low) or 2 (high), when counterbalacing = 1
...
% calculate and display percentage of each button 
