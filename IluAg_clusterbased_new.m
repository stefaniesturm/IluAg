...
% created by Marta for SGMem, modified by Nad for SGMem2
...
[test] = ft_timelockstatistics(cfg, POS{:}, NEG{:}); 
save ([indir 'testdecember.mat'], 'test')

%just change the latency to see P2 window
cfg.latency = [0.18 0.3];
[ERP_eAvseMA_eM_P2] = ft_timelockstatistics(cfg, C1{:}, C2{:}); 
save ([indir 'ERP_eAvseMA_eM_P2.mat'], 'ERP_eAvseMA_eM_P2')
...
cd(indir); load GA_eA_ERP; load GA_eMA_eM_ERP;

cfg = [];
cfg.operation = 'subtract';
cfg.parameter = 'avg';
GA_eA_vs_eMA_eM = ft_math(cfg,GA_eA_ERP, GA_eMA_eM_ERP)

figure;
% define params for plotting 
timestep = 0.03 %in secs
sampling_rate = 500; %
sample_count = length(ERP_eAvseMA_eM_N1.time);
% j and M must have the same temporal length
j = [ERP_eAvseMA_eM_N1.time(1):timestep:ERP_eAvseMA_eM_N1.time(end)]; %temporal endpoints (in secs) of the ERP avg computed in each subplot
m = [1:timestep*sampling_rate:sample_count]; %temporal endpoints in EEG samples
% get relevant (significant) values for positive
pos_cluster_pvals = [ERP_eAvseMA_eM_N1.posclusters(:).prob];
pos_signif_clust = find(pos_cluster_pvals < ERP_eAvseMA_eM_N1.cfg.alpha);
pos = ismember(ERP_eAvseMA_eM_N1.posclusterslabelmat, pos_signif_clust);

% get relevant (significant) values for negative
neg_cluster_pvals = [ERP_eAvseMA_eM_N1.negclusters(:).prob];
neg_signif_clust = find(neg_cluster_pvals < ERP_eAvseMA_eM_N1.cfg.alpha);
neg = ismember(ERP_eAvseMA_eM_N1.negclusterslabelmat, neg_signif_clust);
...
% the order
[i1,i2] = match_str(GA_eA_vs_eMA_eM.label, ERP_eAvseMA_eM_N1.label);
...
for k = 1:length(m)-1
   ...
   % cfg.zlim = [-5e-14 5e-14];
    ...
    pos_int = zeros(numel(GA_eA_vs_eMA_eM.label),1);
    ...
    cfg.interactive ='no';
    cfg.layout = layout;
    ft_topoplotER(cfg,GA_eA_vs_eMA_eM);
end
